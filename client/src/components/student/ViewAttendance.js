...
const ViewAttendance = () => {  
    ...
    // The :id here actually represents the courseId
    const { id } = useParams();
    const courseId = id;
    ...
    useEffect(() => {
        const fetchData = async() =>{
            try{
                ...
                console.log('Attendance records:', resAttendance.data);
                setAttendanceRecords(resAttendance.data);

                const resStudent = await axios.get(`${url}/api/students`);
                console.log('All students:', resStudent.data);
                ...
                if (studentData) {
                    ...
                    const records = resAttendance.data;
                    const totalClasses = records.length;
                    let attendedClasses = 0;
                    
                    records.forEach(record => {
                        const studentRecord = record.students.find(
                            s => s.student && s.student._id === studentData._id
                        );
